...
public class DelimiterMatching {
    ...
    public static boolean doesMatch(String testString) {
        ...
        for (int i = 0; i < testString.length(); i++) { //Construct a for loop to itterate through the length of the input string
            char current = testString.charAt(i); //Create a variable 'current' that hold the character at the "ith"/ current position 
            
            if (current == '(' || current == '[' || current == '{') { //If the value at that index is an open symbol listed...
                delimiterStack.push(current); // We are going to push it onto the stack we created
            } 
    ...
    public static void main(String[] args) {
        ...
        //Check to see if the users string is matching by calling the doesMatch method, passing the testString through, and printing the results
